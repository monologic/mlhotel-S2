...
<body>
  ...
  <p>Código de Reserva: {{ $codigo_reserva }}</p>
	...
	<p>Fecha Salida o Check out: {{ $fecha_fin }}</p>
  	...
  	<p>
  		El pago de la reserva, tal como lo solicitó será efectuada vía depósito bancario en nuestra cuenta en el Banco <strong>{{ $banco['banco'] }}</strong> Nro de cuenta <strong>{{ $banco['cuenta'] }}</strong>,con CCI <strong>{{ $banco['cci'] }}</strong> por el importe de S/ {{$total_pagado}}. Le rogamos se sirva enviarnos el voucher del depósito a efectos de confirmar su reserva vía correo electrónico a la dirección {{$hotel['correo']}}.
  	</p>
